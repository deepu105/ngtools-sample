...
currentAccount: any;
    entityWithServiceClassPaginationAndDTOS: EntityWithServiceClassPaginationAndDTO[];
    error: any;
    success: any;
    ...
    loadAll() {
        if (this.currentSearch) {
            this.entityWithServiceClassPaginationAndDTOService.search({
        ...
        this.entityWithServiceClassPaginationAndDTOService.query({
            page: this.page - 1,
            size: this.itemsPerPage,
            sort: this.sort()}).subscribe(
            (res: ResponseWrapper) => this.onSuccess(res.json, res.headers),
    ...
    transition() {
        this.router.navigate(['/entity-with-service-class-pagination-and-dto'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: EntityWithServiceClassPaginationAndDTO) {
        return item.id;
    }
    registerChangeInEntityWithServiceClassPaginationAndDTOS() {
        this.eventSubscriber = this.eventManager.subscribe('entityWithServiceClassPaginationAndDTOListModification', (response) => this.loadAll());
    }
