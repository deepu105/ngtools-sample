...
currentAccount: any;
    entityWithServiceImplAndPaginations: EntityWithServiceImplAndPagination[];
    error: any;
    success: any;
    ...
    loadAll() {
        if (this.currentSearch) {
            this.entityWithServiceImplAndPaginationService.search({
        ...
        this.entityWithServiceImplAndPaginationService.query({
            page: this.page - 1,
            size: this.itemsPerPage,
            sort: this.sort()}).subscribe(
            (res: ResponseWrapper) => this.onSuccess(res.json, res.headers),
    ...
    transition() {
        this.router.navigate(['/entity-with-service-impl-and-pagination'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: EntityWithServiceImplAndPagination) {
        return item.id;
    }
    registerChangeInEntityWithServiceImplAndPaginations() {
        this.eventSubscriber = this.eventManager.subscribe('entityWithServiceImplAndPaginationListModification', (response) => this.loadAll());
    }
