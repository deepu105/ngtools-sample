...
currentAccount: any;
    entityWithServiceClassAndPaginations: EntityWithServiceClassAndPagination[];
    error: any;
    success: any;
    ...
    loadAll() {
        if (this.currentSearch) {
            this.entityWithServiceClassAndPaginationService.search({
        ...
        this.entityWithServiceClassAndPaginationService.query({
            page: this.page - 1,
            size: this.itemsPerPage,
            sort: this.sort()}).subscribe(
            (res: ResponseWrapper) => this.onSuccess(res.json, res.headers),
    ...
    transition() {
        this.router.navigate(['/entity-with-service-class-and-pagination'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: EntityWithServiceClassAndPagination) {
        return item.id;
    }
    registerChangeInEntityWithServiceClassAndPaginations() {
        this.eventSubscriber = this.eventManager.subscribe('entityWithServiceClassAndPaginationListModification', (response) => this.loadAll());
    }
