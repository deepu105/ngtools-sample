    ...
    open(component: Component, id?: number | any): NgbModalRef {
        ...
        if (id) {
            this.bankAccountService.find(id).subscribe((bankAccount) => {
                if (bankAccount.openingDay) {
                    bankAccount.openingDay = {
                        year: bankAccount.openingDay.getFullYear(),
                        month: bankAccount.openingDay.getMonth() + 1,
                        day: bankAccount.openingDay.getDate()
                    };
                }
                bankAccount.lastOperationDate = this.datePipe
                    .transform(bankAccount.lastOperationDate, 'yyyy-MM-ddThh:mm');
